...
const api = {
  ...
  chatSend: (data: { text: string; systemPrompt?: string; workspaceDir?: string | null; images?: Array<{ mimeType: string; base64: string }>; documents?: Array<{ name: string; content: string; language?: string }> }) => ipcRenderer.invoke('chat-send', data),
  chatStop: () => ipcRenderer.invoke('chat-stop'),
  chatReset: () => ipcRenderer.invoke('chat-reset'),
  modelDisconnect: () => ipcRenderer.invoke('model-disconnect'),
  approveTool: (approved: boolean) => ipcRenderer.invoke('approve-tool', approved),
  updateWorkspaceDir: (dir: string | null) => ipcRenderer.invoke('update-workspace-dir', dir),
  fetchModels: (apiUrl: string, apiKey?: string) => ipcRenderer.invoke('fetch-models', apiUrl, apiKey),
